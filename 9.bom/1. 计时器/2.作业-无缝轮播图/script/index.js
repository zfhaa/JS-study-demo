...
function initElements() {
    ...
    var newImg = children[0].cloneNode(true); //深度克隆
    config.doms.divImgs.appendChild(newImg);
    newImg = children[children.length - 2].cloneNode(true);
    config.doms.divImgs.insertBefore(newImg, children[0]);
...
function switchTo(index, direction) {
    ...
    function animateSwitch() {
        ...
        //3.计算每次改变的距离
        var everyDistancce = distance / number;
        config.timer.id = setInterval(function () {
            //改变div的marginleft
            marginLeft += everyDistancce;
...
config.doms.divDots.onclick= function(e){
    if(e.target.tagName==="SPAN"){
        var index = Array.from(this.children).indexOf(e.target);//拿到点击的圆点的下标
